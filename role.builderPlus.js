 ...
 * var mod = require('role.builderPlus');
...
var state = 'spawned';
var closestSource = '576a9cb857110ab231d899fa';

/*
        if(isNotAssigned) {
            //assign left or right mine, randomly
            var random = Math.round(Math.random()*3.33);
            
            if(random % 2==0) {
                 closestSource= leftMine;
            } else {
                 closestSource = rightMine;
            }
            isMineAssigned = false;
        } 
    ...
    run: function(creep) {
	    ...
	    if(creep.memory.building) {
	        ...
	        var sources = creep.room.find(FIND_SOURCES);
	          ...
	          console.log('builder '+creep.name+ ' moves to + '+closestSource);
                if(creep.harvest(closestSource)==ERR_NOT_IN_RANGE) {
                    creep.moveTo(closestSource);
                    console.log('builder '+creep.name + ' is harvesting');
...
module.exports = roleBuilderPlus;
