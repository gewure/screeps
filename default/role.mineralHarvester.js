...
var roleMineralHarvester = {
    ...
    run: function(creep, sourceID, containerIDs, linkID, minerID) {
        ...
        if(creep.ticksToLive > 10) {
            ...
            else creep.transfer(link, RESOURCE_LEMERGIUM);
        }
	}
};
...
function fillLink(creep, stateChanged) {
        ...
        var ret = creep.transfer(link, RESOURCE_LEMERGIUM, creep.carry.L);
        console.log('retval of transfer:' + ret);
        console.log(creep.carry.L + '  ' + link);
    if(creep.transfer(link, RESOURCES_ALL) == ERR_NOT_IN_RANGE) {
...
function fillContainer(creep, stateChanged, cont) {
    ...
    if(creep.transfer(cont,RESOURCE_LEMERGIUM) == ERR_NOT_IN_RANGE) {
