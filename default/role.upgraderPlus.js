...
var roleUpgraderPlus = { // IS A UPGRADER NOW

    /** @param {Creep} creep **/
    run: function(creep,flagId, workRoom) {
        homeRoom = workRoom;
        homeFlag = flagId;
        ...
        if(creep.memory.upgrading) { // upgrading
            ...
            */
            if(false && droppedRes && (creep.carry.energy < creep.carryCapacity*0.6) ) {
                ...
                if(creep.pickup(droppedRes)==ERR_NOT_IN_RANGE) {
                 ...
                 if(creep.pos.roomName != homeRoom) { // this may happen, sadly
                    creep.say('home!');
                    creep.moveTo(Game.flags[homeFlag], {reusePath:5});
                }
            } else {
...
module.exports = roleUpgraderPlus;