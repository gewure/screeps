...
var contStorIDs = ['57796949720916567dc376ca', '5779b1268eef705e48c33a6e', '5779cfd7986f591c34eadf9a', '577acd04d33b4b4d0f4bf616']; //'577af4e8502328d4276d1a7d', 
...
var towerIDs = ['578165832b0f16cb03ea73e4']; //TODO not yet used


var container = undefined;
var untilPathRecalc = 2;
var reFillFactorFullContainer = 40;
    ...
    */
    ...
    run: function(creep, contIDs, contStorIDs, towIDs) {
        ...
        if(closestHostiles.length > 0) {
            creep.memory.reFillFaktorFullContainer = 30;
            creep.memory.minEnergyLimit = 50;
        } else {
            creep.memory.reFillFactorFullContainer = 30;
            creep.memory.minEnergyLimit = minEnergyLimit;
        }
           ...
           if(_.sum(creep.carry) == creep.carryCapacity) {
               ...
               /*
                ...
                if(stor) {
                 ...
                 var fillTarget = getClosestLowEnergyTower(creep);
                 
                 if(_.sum(creep.carry) > creep.carry.energy) { //RESOURCES!
                 ...
                 } else if(!creep.pos.isNearTo(fillTarget)) {
                        creep.moveTo(fillTarget);
                    } else { // is near storage, transfer _all_ resources
                        creep.transfer(fillTarget, RESOURCE_ENERGY); 
            ...
            var droppedRes = creep.pos.findClosestByRange(FIND_DROPPED_RESOURCES, {
                        filter: (resource) => {
                            return (resource.amount > creep.pos.getRangeTo(resource) *1.3);
                        }, algorithm:'dijkstra'
                });
            ///console.log(droppedRes);
            creep.memory.dropedRes= droppedRes;
            if(!droppedRes) { 
                isCollectMode = false;
            ...
            if(droppedRes && _.sum(creep.carry)<= 0.2*creep.carryCapacity) {
                ...
                if(_.sum(creep.carry) < creep.carryCapacity) {
                    if(creep.pickup(droppedRes)==ERR_NOT_IN_RANGE) {
                        ...
                        var path = creep.room.findPath(creep.pos, droppedRes.pos, {
                ...
                var target = Game.getObjectById(storageID);
...
function getClosestHighEnergyContainer(creep, minEnergyLimit, idArray) {
    ...
    for(var i = 0; i < idArray.length; ++i) {
        ...
        if(_.sum(container.store) > creep.memory.minEnergyLimit) 
            closestArray[closestArray.length] = container;
    }
    var closest = creep.pos.findClosestByRange(closestArray);
    return closest;
...
module.exports = roleOtherRoomCollector;